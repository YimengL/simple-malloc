...
void* mm_malloc (size_t size) {
	size_t reqSize;			/* adjusted block size */
	...
	BlockInfo* nextBlockFollowing = NULL;
	...
	blockSize= SIZE(ptrFreeBlock->sizeAndTags);		/* get the free blcok size size */
	...
	if (blockSize - reqSize < MIN_BLOCK_SIZE) {
		...
		/* remove the allocated memory from the free list. */
		removeFreeBlock(ptrFreeBlock);
	}
	else {
		/* splitting case */
		/* next block is also a free block */
		
		/* Obtain the current TAG_PRECEDING_USED, for the new free block */
		precedingBlockUseTag = (ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED);
		
		/* set the size and tag for the new free block */
		ptrFreeBlock->sizeAndTags = (reqSize | precedingBlockUseTag | TAG_USED);
		
		removeFreeBlock(ptrFreeBlock);
		
		/* obtain a pointer to the following block */
		nextBlock = (BlockInfo*)POINTER_ADD(ptrFreeBlock, reqSize);
		nextBlock->sizeAndTags = (blockSize -reqSize) | TAG_PRECEDING_USED;		/* Its header */
		/* set its footer */		
		nextBlock->sizeAndTags = (blockSize -reqSize) | TAG_PRECEDING_USED;
		*(size_t*)POINTER_ADD(nextBlock, SIZE(nextBlock->sizeAndTags) - WORD_SIZE) 
...
void mm_free (void *ptr) {
	...
	/* turn off the blockinfo tag */
	blockInfo->sizeAndTags &= (~TAG_USED);
	
	/* turn off the preceding used tag of the following block */
	followingBlock->sizeAndTags &= (~TAG_PRECEDING_USED);
	
	/* Add a footer to the blockinfo */
	*(size_t*)POINTER_ADD(blockInfo, SIZE(blockInfo->sizeAndTags) - WORD_SIZE) 
